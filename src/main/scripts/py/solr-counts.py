...
indexName = 'dare2'
req = urllib2.Request('http://localhost:8983/solr/' + indexName + '/select?wt=python&q=*:*&rows=0&facet=on&facet.field=source_s')
...
for i in range(0, len(sources), 2):
    req = urllib2.Request('http://localhost:8983/solr/' + indexName + '/select?wt=python&q=*:*' +
    ...
    for j in range(0, len(genres), 2):
        if genres[j+1] > 0:
            ...
            req = urllib2.Request('http://localhost:8983/solr/' + indexName + '/select?rows=0&wt=python&q=*:*' +
                                  '&fq=source_s:%22' + urllib2.quote(sources[i]) + '%22' +
